...
public class ATKSpeedUPBuff : GatchaSkill
{
    //캐릭터 버프시 필요한것
    ...
    //강화 전 데미지 
    private List<float> originATKSpeed = new List<float>();
    private List<float> rivalOriginATKSpeed = new List<float>();
        ...
        if (InGameInfoManager.Instance.isPVPMode)
        {
            if (!isRivalSkill)
            {
                ...
                for (int i = 0; i < PVPCharManager.Instance.summonList.Count; i++)
                {
                    ...
                    //타겟에게 맞는 버프 적용 
                    originATKSpeed.Add(PVPCharManager.Instance.summonList[i].attackDelayTime);
                    //버프 적용 (현재 데미지 + (초기 데미지 * 강화 비율))
                    PVPCharManager.Instance.summonList[i].attackDelayTime -= PVPCharManager.Instance.summonList[i].charData.attackSpeed * enhanceRatio;
                    //버프 이펙트 활성화 
    ...
    public override void RivalDoSkill(int[] targetNums)
    {
        ...
        for (int i = 0; i < targetNums.Length; i++)
        {
            ...
            //타겟에게 맞는 버프 적용 
            rivalOriginATKSpeed.Add(PVPInGM.Instance.activeUnits[targetNums[i]].attackDelayTime);
            //버프 적용 (현재 데미지 + (초기 데미지 * 강화 비율))
            PVPInGM.Instance.activeUnits[targetNums[i]].attackDelayTime -= PVPInGM.Instance.activeUnits[targetNums[i]].charData.attackSpeed * enhanceRatio;
            //버프 이펙트 활성화 
        ...
        if (InGameInfoManager.Instance.isPVPMode)
        {
            ...
            for (int i = 0; i < charactors.Count; i++)
            {
                for (int j = 0; j < PVPCharManager.Instance.summonList.Count; j++)
                {
                    ...
                    {
                        charactors[i].attackDelayTime = originATKSpeed[i];
                    }
                }
            ...
            for (int i = 0; i < rivals.Count; i++)
            {
                for (int j = 0; j < RivalManager.Instance.summonList.Count; j++)
                {
                    ...
                    {
                        rivals[i].attackDelayTime = rivalOriginATKSpeed[i];
                    }
                }
            ...
            charactors.Clear();
            rivals.Clear();
            originATKSpeed.Clear();
            rivalOriginATKSpeed.Clear();
